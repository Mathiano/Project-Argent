...
## PHASE 7 — Content authoring tools + Chapter 1 content ✅ GATE MET (re-audited 2026-09-20) — CHAPTER 1 COMPLETE
...
> - ✅ **The CH1-COMPLETE checklist is now CLOSED (re-audited against the repo, 2026-09-20).** All three items below had SHIPPED without this list being updated — it was stale for months and mis-stated the frontier. Verified by running the suites named, not by reading docs:
>   1. ~~**Rival-fight integration**~~ ✅ **SHIPPED** — the KAMON first-fight ships as the Violet→Route 32 gate (post-ZEPHYR): the chokepoint obstacle, KAMON's post-badge spawn, the once-only fire, and the exit opening after `kamon_beaten`. `src/game/scenes/violetGate.test.ts` (9 tests); wiring in `main.ts` (`showRivalBattle` + the gate). Card fairness is sim-gated separately in `src/sim/rivalCard.test.ts`.
>   2. ~~**CH1 story beats + full Violet population**~~ ✅ **SHIPPED** — Violet carries **16 NPCs / 8 signs / 6 warps**; Hearthwick 16 entities; the gym 5 NPCs + gust pulses + its script. The chapter close is authored and tested: the KAMON gate exchange, the Route 32 quiet-resolve, and the CHAPTER ONE — *Kindled* card (`ch1Ending.ts`, fires once on `ch1_closed`).
>   3. ~~**Real Violet Academy hub**~~ ✅ **SHIPPED** — not a stub: 5 teaching NPCs + a sign, FALKNER's in-gym mentor line on the win, and the `academy_promoted` gate. `violetAcademy.test.ts` (9) + `academyEntrance.test.ts` (5).
>
> **⚠️ Process note (why this matters more than the three items):** this checklist stayed stale while the work shipped, and a later planning pass read it as truth and re-proposed built features. The same happened in `combat-enrichment-roadmap.md` (Layer 3.5's foe-★ default, reversed in playtest and never back-annotated). **Before planning off any roadmap line here, check it against the test suite.** Status lines in this file are only as good as their last audit.
...
> **Roster design status (`docs/mon-manifest.csv`):** the active main-game roster is **designed through Batch 9** — ~172 named rows (78 lines) with concept notes, including the **4 legendaries** (L004–L007: MAREGLACE/Fluid, FERRUMTHANE/Guard, PERMAFROIX/Aggressive stance-sovereigns + **ARGENTYR**, the cover moon-dragon myth). **Legendaries designed. Remaining reserved slots are named per-chapter as content builds — NOT pre-named:** the **Johto-filler** slots (L010–L082 — ilex forest, dojo isle, ranch fields, etc.) get named against the real chapter as each Johto chapter builds; the **Kanto + lost-region/postgame** slots (L083–L105) are Part-2 content, reserved until that region builds.
...
- Real Route 31 + Violet with proper trainers, the rival's first fight (KAMON v2 card — already queued), the first Rocket beat, ~~gym trainers feeding scout reports~~ ✅ **SHIPPED 2026-09-21** — the scout-report economy (`src/game/scout.ts`): Falkner's sheet is six data-driven lines, each derived from the LIVE boss card and bought with a trainer's win-flag, with two deliberately skippable sellers (the gym trainer off the sight-lines, and WREN a route away) so a beeline arrives at 4/6. Code-truth: `docs/scout-report-AS-BUILT.md`. *(It also retired a falsehood: the old hardcoded sheet advertised "Break bar 2" against a `breakBar: 4` card.)*
...
- ✅ **Combat Layer 3 — environments** — arena rhythm / Falkner's gust beats (`arena.test.ts`, `bossAI.test.ts`) PLUS the field-environment system (2026-09-20): 7 biomes tilting stance damage/mitigation, FOCUS releases and the STAMINA economy, with the terrain meta-read finally reading `TrainerProfile.terrain`. Sim-gated per the roadmap's Monte Carlo TODO (`src/sim/environmentBalance.test.ts`), which caught a genuine in-biome dominant strategy mid-build. Code-truth + findings: `docs/combat-environments-AS-BUILT.md`. **Not wired to maps yet** — the engine supports it, no content sets it.
...
- **Sprite production method is now DEFINED** (the pipeline is discoverable, not ad-hoc): `docs/sprite-pipeline-roles.md` — the four-agent roles + end-to-end workflow (Chat/Fable concept + real-animal-anchored brief, Claude Design palette/structure only, Gemini the 56px art, CC ingests via `sprite_ingest.py` + manifest↔art consistency, Mathias per-mon veto); `docs/gemini-sprite-briefing.md` — the standing governance brief handed to Gemini (aesthetic law + forbidden list, GBC format spec, archetype→shape language, one-at-a-time + sign-off rules, worked L001 example). The **L001 pilot established the ANTI-DRIFT METHOD** (real-animal anchoring + "NOT this" lists), after prose drifted to a bear and a drawn silhouette drifted to a hippo — both docs encode it.
...
- ~~**Seeded overworld RNG (replays / Gauntlet / determinism).**~~ ✅ **ALREADY DONE (re-audited 2026-09-23)** — `scenes/overworld.ts` takes a REQUIRED `random: () => number` that main.ts feeds from the run's seeded rng, with **no `Math.random` fallback** ("a caller without a seed must fail to compile rather than silently going non-deterministic"). Covered by `encounterRng.test.ts`. The paragraph below is the original, kept for its rationale; its premise is stale. *(Second stale backlog line found this way — check a roadmap claim against the suite before planning off it.)*
- ~~*(original entry)*~~ Overworld encounter rolls (`Math.random() < zone.rate` in `scenes/overworld.ts`) bypass the seeded `mulberry32` the engine uses. Save/load (Phase 2) stores the seed; loading reseeds; but encounter-zone rolls aren't reproducible across save boundaries. Land before the Gauntlet / replay features (Phase 8) so seed-based bug repros and "this exact encounter sequence" mods are possible. Lift is small — route encounter rolls through `run.rng`; persisting its internal counter is a `version: 2` save migration.

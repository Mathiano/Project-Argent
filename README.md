...
1. ~~**Starter-trio bulk rebalance.**~~ ✅ **DONE (2026-06-21).** Contained fix per `docs/starter-trio-rebalance.md` (RESOLUTION): the trio re-shaped to a shared budget 330 (GRUBLEAF softened Dodger→fast-bruiser, SILTSKIP's bulk reined, KINDRAKE held). Mirror-sim now lands **52.1 / 49.7 / 47.4%** RPS (spread 4.7pp, gated by `starterMirror.test.ts`); KAMON's per-pick levels re-converged to ~1.0 (0.90–1.03) at **72.0 / 70.3 / 65.7%** (fairness gates restored); Falkner cells re-locked. **🏦 Banked:** the systemic SPD/ATK-vs-HP/DEF exchange-rate finding (the pure-Dodger archetype is soft at parity) — revisit only when offense/initiative archetypes come online, not on this one data point.

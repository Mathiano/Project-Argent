...
- ★ Momentum: +1 on read-wins (counter landed, opening landed, **punish landed (A>F)**, clash won), cap 2. **Calls** (spend 1 ★): GET AWAY (negate the incoming hit), HANG IN THERE (can't drop below 1 HP), Catch Breath (**+50 ST** — `catchBreathRestorePct` 0.5 of the 100-ST cap; was +35, re-baselined Phase 6b). **Bond JUMPSTART** (engine hook `jumpstartArmed`): a Familiar-tier mon's FIRST read-win banks +1 extra ★, once per battle

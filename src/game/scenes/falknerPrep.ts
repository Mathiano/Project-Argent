// Falkner's prep screen — a RENDER of the scout-report economy (`src/game/scout.ts`),
// not a wall of authored text.
//
...
import { bossScoutFacts, buildScoutReport, FALKNER_REPORT } from '../scout';
...
export function createFalknerPrepScene(opts: FalknerPrepSceneOpts): Scene {
  ...
  const report = buildScoutReport(FALKNER_REPORT, facts, opts.hasFlag);
  ...
  return {
    ...
    draw(ctx) {
      ...
      // FREE facts head the sheet, full width — you can see the bird yourself.
      free.forEach((e, i) => {

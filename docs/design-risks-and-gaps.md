...
11. ~~**Save migrator (CC audit) — before players, not now.**~~ ✅ **BUILT 2026-09-23.** `save.ts` now carries `CURRENT_SAVE_VERSION` + a `SAVE_MIGRATIONS` chain (`vN → vN+1`, each step pure and total), walked by `migrateSave` before validation. The chain is empty — v1 is still current — but it exists so a bump is a step, not a decision about discarding saves; `save.test.ts` exercises it with a synthetic two-step chain, a chain GAP, and a newer-than-known save.
    **The sharper half of this risk turned out not to be the migrator.** A save the loader rejected was treated as NO save: CONTINUE was hidden, the player pressed New Game, and `autosaveNow()` — which fires on every overworld transition — wrote over it within seconds. A bad byte silently cost a run. Now a rejected blob is **quarantined** (`QUARANTINE_KEY`, with the reason and a timestamp) and the live slot cleared before anything can overwrite it, the title **loads once up front** rather than probing with `hasSave()` (which only checked that the key existed, so a corrupt save used to offer CONTINUE), and the player is told: *SAVE COULD NOT BE READ — KEPT ASIDE, NOT DELETED*. A `future`-version save is preserved verbatim, never downgraded.
    Still open: nothing in-game *restores* from quarantine (the bytes are recoverable by hand only), and the Nuzlocke/permadeath autosave-mode decision below is untouched.

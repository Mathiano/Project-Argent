...
| **`overworld-party`** | **Overworld at ROUTE31 with a 3-mon party (GRUBLEAF + KINDRAKE + SILTSKIP)** | **Phase 4 hook — exercises the pause/party menus + reorder out of the box.** *(Stale since the JAY beat + guided catch landed — noted 2026-09-24: at the ROUTE31 default spawn JAY's approach runs and a guided-catch FLITPECK battle starts, so START doesn't reach the pause menu. For the pause/party menus use `?skip=violet&party=…`.)* `?party=A,B,C` overrides the default roster. Phase 5a: also seeds the default 3 POTIONs, and `?bag=` overrides the seeded inventory. Sets `player_has_starter` so the south gate is open. |

...
| **`route31-big`** | **Route 31 Phase 1 — the full 22×74 canvas imported+wired, walked in-engine** | **Terrain incl. WATER + collision + warps (N→Hearthwick, S→Violet) + 7 encounter zones + tree-top walk-behind (Overhead layer). Enters from the north (`fromHearthwick`). NPCs are placeholders (Jay next). `?party=`/`?starter=` for the trainer. See `docs/route31-migration-scope.md`.** |
